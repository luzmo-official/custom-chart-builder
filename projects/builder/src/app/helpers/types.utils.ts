...
export function isInt(value: unknown): value is number {
  return typeof value === 'number' && !Number.isNaN(value) && Number.parseInt(value.toString(), 10) === value;
}
...
export function isUUID(value: unknown): boolean {
  return isString(value) && /^[\dA-Fa-f]{8}-[\dA-Fa-f]{4}-4[\dA-Fa-f]{3}-[89ABab][\dA-Fa-f]{3}-[\dA-Fa-f]{12}$/.test(value);
}
...
export function isFunction(value: unknown): value is Function {
  return value !== null && value !== undefined && Object.prototype.toString.call(value) === '[object Function]';
}
...
export function isISO8601DateString(date: unknown): boolean {
  return isString(date) && /(\d{4}-[01]\d-[0-3]\dT[0-2](?:\d:[0-5]){2}\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2](?:\d:[0-5]){2}\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/.test(date);
}
