...
function hexbin_default() {
  ...
  function hexbin(points) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      var id2 = pi + "-" + pj, bin = binsById[id2];
      if (bin) bin.push(point);
      else {
        bins.push(bin = binsById[id2] = [point]);
        bin.x = (pi + (pj & 1) / 2) * dx;
        bin.y = pj * dy;
      }
    }
...
var render = (rootElement, data) => {
  ...
  let canvasSelection = select_default2(rootElement).select("canvas");
  if (canvasSelection.empty()) {
    canvasSelection = select_default2(rootElement).append("canvas").attr("width", defaultWidth).attr("height", defaultHeight).style("display", "block");
  }
  const canvas = canvasSelection.node();
  const ctx = canvas.getContext("2d");
  canvas.__data__ = data;
  ...
  const xScale = linear2().domain(xExtent).nice().range([margin.left, width - margin.right]);
  const yScale = linear2().domain(yExtent).nice().range([height - margin.bottom, margin.top]);
  const hexbin = hexbin_default().x((d) => xScale(d[0])).y((d) => yScale(d[1])).radius(9).extent([[margin.left, margin.top], [width - margin.right, height - margin.bottom]]);
  const bins = hexbin(data);
  const maxCount = max(bins, (d) => d.length) || 1;
  const totalClicks = data.length;
  const colorScale = sequential(viridis_default).domain([0, maxCount]);
  ...
  function drawCanvas() {
    ...
    const bgImg2 = canvas._bgImg;
    if (bgImg2 && bgImg2.complete) {
      ctx.save();
      ctx.globalAlpha = 0.85;
      ctx.drawImage(bgImg2, 0, 0, width, height);
      ctx.restore();
    }
    bins.forEach((bin) => {
      ctx.save();
      ctx.fillStyle = colorScale(bin.length);
      ctx.globalAlpha = bin.currentOpacity !== void 0 ? bin.currentOpacity : 0.3 + 0.7 * (bin.length / maxCount);
  ...
  function animateOpacityTransition(duration) {
    ...
    timer(function() {
      ...
      t = cubicOut(t);
      bins.forEach((bin) => {
        if (bin.startOpacity !== void 0 && bin.targetOpacity !== void 0) {
  ...
  let tooltip = select_default2(rootElement).select(".tooltip");
  if (tooltip.empty()) {
    tooltip = select_default2(rootElement).append("div").attr("class", "tooltip").style("position", "absolute").style("pointer-events", "none").style("background-color", "#000").style("color", "#fff").style("padding", "8px 12px").style("border-radius", "6px").style("font-family", "Inter, sans-serif").style("font-size", "14px").style("line-height", "20px").style("opacity", 0);
  }
  let clearSelectionBtn = select_default2(rootElement).select(".clear-selection-btn");
  if (clearSelectionBtn.empty()) {
    clearSelectionBtn = select_default2(rootElement).append("div").attr("class", "clear-selection-btn").style("position", "absolute").style("pointer-events", "auto").style("background-color", "#000").style("color", "#fff").style("padding", "6px 8px").style("border-radius", "6px").style("font-family", "Inter, sans-serif").style("font-size", "12px").style("opacity", 0).style("cursor", "pointer").text("Clear selection").on("click", function(event) {
  ...
  function updateClearSelectionButtonPosition() {
    ...
    clearSelectionBtn.style("left", `${canvasLeft + 10}px`).style("top", `${canvasTop + 10}px`);
  }
  updateClearSelectionButtonPosition();
  select_default2(window).on("scroll.renderClearBtn", updateClearSelectionButtonPosition).on("resize.renderClearBtn", updateClearSelectionButtonPosition);
  ...
  const canvasSel = select_default2(canvas);
  canvasSel.on("mousemove.render", null).on("mouseout.render", null).on("click.render", null);
  canvasSel.on("mousemove.render", function(event) {
    const mousePos = getMousePos(event);
    const foundBin = bins.find(
      (bin) => bin.path ? ctx.isPointInPath(bin.path, mousePos.x, mousePos.y) : false
    );
    if (foundBin) {
      const X2 = xScale.invert(foundBin.x);
      const Y2 = yScale.invert(foundBin.y);
      const percent = (foundBin.length / totalClicks * 100).toFixed(2);
      tooltip.style("opacity", "0.925").html(
        `<b>Coordinates</b>: (${X2.toFixed(2)}, ${Y2.toFixed(2)})<br/>
             <b>Click count</b>: ${foundBin.length} (${percent}% of total)`
      ).style("left", event.pageX + 7.5 + "px").style("top", event.pageY + 7.5 + "px");
    ...
    if (clickedBin) {
      ...
      bins.forEach((bin) => {
        bin.targetOpacity = selectionExists ? bin.selected ? 1 : 0.25 : 0.3 + 0.7 * (bin.length / maxCount);
      });
      animateOpacityTransition(300);
...
var resize = (rootElement, resizedWidth, resizedHeight) => {
  const canvasSelection = select_default2(rootElement).select("canvas");
  ...
  const clearSelectionBtn = select_default2(rootElement).select(".clear-selection-btn");
  if (!clearSelectionBtn.empty()) {
    const { left, top } = canvas.getBoundingClientRect();
    clearSelectionBtn.style("left", `${left + window.pageXOffset + 10}px`).style("top", `${top + window.pageYOffset + 10}px`);
  }
};
export {
  render,
